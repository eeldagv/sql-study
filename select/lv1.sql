...
-- 틀린 이유: 인덱스랑 비트 자리는 다른 규칙이다.
-- 비트 자리는 1부터 시작, 자리값은 인덱스가 아니라 2의 거듭제곱 값
-- 1번째 자리 -> 2^0 = 1
-- 2번째 자리 -> 2^1 = 2
-- 3번째 자리 -> 2^2 = 4
-- 4번째 자리 -> 2^3 = 8
-- 또한, 1 IN (GENOTYPE & 1, GENOTYPE & 3)은 결과가 정확히 1이어야만 통과되는데 정답인 & 4로 고치면 3번 형질이 있을 때 결과가 4가 나오므로 매칭이 안 되므로, '0이 아니다 = 형질이 있다' 를 확인해야 한다.
-- 따라서
